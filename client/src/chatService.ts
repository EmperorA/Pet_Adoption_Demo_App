...
export const getChatRoomId = async (userId: string, adminId: string): Promise<string> => {
  try {
    const response = await fetch('https://pawfectmatch-api.onrender.com/api/generate-chat-room-id', {
      method: 'POST',
      headers: {
...
export const sendMessage = async (chatRoomId: string, message: string): Promise<string> => {
  try {
    const response = await fetch('https://pawfectmatch-api.onrender.com/api/send-message', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('token')}`
...
export const getMessages = async (chatRoomId: string): Promise<Message[]> => {
  try {
    ...
    const response = await fetch(`https://pawfectmatch-api.onrender.com/api/messages/${chatRoomId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
